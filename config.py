...
class Settings:
    ...
    app_time_display_mode: str = os.getenv("APP_TIME_DISPLAY_MODE", "client-local").strip().lower()
    app_security_audit_enabled: bool = _env_bool("APP_SECURITY_AUDIT_ENABLED", "true")
    app_resume_retry_max: int = int(os.getenv("APP_RESUME_RETRY_MAX", "2"))
    app_resume_retry_backoff_sec: int = int(os.getenv("APP_RESUME_RETRY_BACKOFF_SEC", "2"))
    ...
    proxy_server: str = os.getenv("EKHNP_PROXY_SERVER", "").strip()
    proxy_username: str = os.getenv("EKHNP_PROXY_USERNAME", "").strip()
    proxy_password: str = os.getenv("EKHNP_PROXY_PASSWORD", "").strip()
    proxy_required: bool = _env_bool("EKHNP_PROXY_REQUIRED", "false")
    proxy_country: str = os.getenv("EKHNP_PROXY_COUNTRY", "KR").strip().upper()
    ...
    rag_generate_model: str = os.getenv("RAG_GENERATE_MODEL", "qwen2.5:3b")
    rag_generate_model_fallbacks: str = os.getenv("RAG_GENERATE_MODEL_FALLBACKS", "qwen2.5:7b,anpigon/eeve-korean-10.8b")
    rag_top_k: int = int(os.getenv("RAG_TOP_K", "6"))
    rag_conf_threshold: float = float(os.getenv("RAG_CONF_THRESHOLD", "0.62"))
    rag_conf_escalate_margin: float = float(os.getenv("RAG_CONF_ESCALATE_MARGIN", "0.08"))
    ...
    rag_low_conf_floor: float = float(os.getenv("RAG_LOW_CONF_FLOOR", "0.53"))
    ...
    rag_negative_evidence_decay_sec: int = int(os.getenv("RAG_NEG_EVIDENCE_DECAY_SEC", "7200"))
    rag_negative_evidence_max_score: float = float(os.getenv("RAG_NEG_EVIDENCE_MAX_SCORE", "6.0"))
    rag_negative_evidence_base_penalty: float = float(os.getenv("RAG_NEG_EVIDENCE_BASE_PENALTY", "0.18"))
    rag_negative_evidence_step_penalty: float = float(os.getenv("RAG_NEG_EVIDENCE_STEP_PENALTY", "0.12"))
    rag_negative_evidence_max_penalty: float = float(os.getenv("RAG_NEG_EVIDENCE_MAX_PENALTY", "0.75"))
