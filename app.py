...
def _render_system_flow_diagram(start_label: str) -> None:
    ...
    st.info("1단계: 계정 동기화\n\nID/PW 입력 -> Enter 또는 로그인/동기화 -> 기존 작업 조회")
    st.caption("↓")
    st.warning(
        f"2단계: 실행 등록\n\n{start_label} 클릭 -> 중복 실행 체크 -> 동시 접속 인원 확인(최대 5명)\n\n"
        "여유 없음: Pending 대기열 / 여유 있음: 즉시 Running"
    )
    st.caption("↓")
    st.success(
        "3단계: 자동화 실행\n\n인덱스 확인/생성 -> 수료 워크플로우(진도 -> 학습시간 -> 시험) 실행\n\n"
        "문항 추출: DOM -> Structured -> OCR 폴백"
    )
    st.caption("↓")
    st.warning("4단계: 판단/우회\n\nAI 문제풀이 역량 부족 시 재질문·재시도·강좌 우회\n\n우회 이력은 재시작 후에도 유지")
    st.caption("↓")
    st.success("5단계: 저장/표시\n\n결과/품질리포트/스냅샷 저장 -> 실시간 로그/상태 반영")
    st.caption("중복 실행 감지 시 새 작업은 차단되고, 기존 작업 상태 화면으로 자동 이동합니다.")
    st.caption(
        "미구현(작게 표시): "
        "예상 대기시간(ETA) 자동 계산, "
