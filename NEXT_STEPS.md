...
- M5 수료 순서 자동화(진도→시간→시험): 92% 진행중
- M6 종합평가 자동화 안정화: 92% 진행중
- M7 LLM(RAG) 기반 시험풀이 고도화: 92% 진행중
- M8 원격 실행 서버화(Streamlit+Tunnel+Worker): 78% 진행중
- M9 동시성 제어(최대 5명) + 대기열: 72% 진행중
- M10 운영(로그/모니터링/복구): 70% 진행중
